...
class Agent(object):
    ...
    @abc.abstractmethod
    def update(self, replay_buffer, logger, step, env):
...
class SACSVGAgent(Agent):
    ...
    def expand_Q(self, state_on, action_on, critic, sample=True, discount=False):
        n_batch = 1
        us, log_p_us, pred_obs = self.dx.unroll_policy(
            state_on, action_on, self.actor, sample=sample, detach_xt=self.actor_detach_rho)
        all_obs = torch.cat((state_on[0, :].unsqueeze(0).unsqueeze(0), pred_obs), dim=0)
        ...
        total_log_p_us = log_p_us.sum(dim=0)
        return total_rewards, first_log_p, total_log_p_us

    def update_actor_and_alpha(self, state_on, action_on, logger, step):
        """
        ...
        rewards, first_log_p, total_log_p_us = self.expand_Q(
            state_on, action_on, self.critic, sample=True, discount=True)
    ...
    def update_critic(self, xs, xps, us, rs, not_done, logger, step):
        ...
        #print("Q_loss",F.mse_loss(current_Q1, target_Q), F.mse_loss(current_Q2, target_Q))
    ...
    def update(self, replay_buffer, logger, step, env):
        ...
        if (self.horizon > 1 or not self.critic) and \
            ...
            for i in range(self.model_update_repeat):
                obses, actions, rewards, train_model = replay_buffer.sample_multistep(
                    self.seq_batch_size, self.seq_train_length)
                if train_model:
                    assert obses.ndimension() == 3
                    dx_loss = self.dx.update_step(obses, actions, rewards, logger, step, train_model)
                    ...
                    self.dx.update_step(obses, actions, rewards, logger, step, train_model)
    ...
    def update_policy_on_policy(self, state_on, action_on, logger, step):
       nu = (state_on.shape[0] - 1) // self.horizon
      # print(state_on.shape, action_on.shape)
       if 1.0 in action_on or -1.0 in action_on:
            return
       for unroll_ind in range ((state_on.shape[0] - 1) // self.horizon):
  #     for unroll_ind in range(1):
            self.update_actor_and_alpha(state_on[unroll_ind*self.horizon:unroll_ind*self.horizon+self.horizon, :],
                                        action_on[unroll_ind*self.horizon:unroll_ind*self.horizon+self.horizon, :], logger, step)
    ...
    def save_checkpoint(self, work_dir=None, tag=None):
        ckpt_path = os.path.join(work_dir, f'{tag}.pth')
        torch.save(
            {
                ...
                "rew_state_dict": self.rew.state_dict(),
                "done_state_dict": self.done.state_dict(),
                "dx_state_dict": self.dx.state_dict(),
            },
            ckpt_path,
        )
        if tag == "best":
            print("Saving the best ckpt to {}".format(ckpt_path))
    ...
    def load_checkpoint(self, ckpt_path, evaluate=False):
        ...
        if ckpt_path is not None:
            ...
            self.rew.load_state_dict(checkpoint["rew_state_dict"])
            self.done.load_state_dict(checkpoint["done_state_dict"])
            self.dx.load_state_dict(checkpoint["dx_state_dict"])
            if evaluate:
                self.actor.eval()
                self.critic.eval()
